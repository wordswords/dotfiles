...
where
author = '$SKYPE_ID' and body_xml LIKE '%Today:%' and date BETWEEN datetime('now', '-15 days') AND datetime('now', 'localtime')
OR
author = '$SKYPE_ID' and body_xml LIKE '%SLS%' and date BETWEEN datetime('now', '-15 days') AND datetime('now', 'localtime')
OR
author = '$SKYPE_ID' and body_xml LIKE '%Yesterday%' and date BETWEEN datetime('now', '-15 days') AND datetime('now', 'localtime')
