...
15. `git checkout <hash> <filepath>` - this will checkout a previous version of the file from the <hash> commit. A useful technique for restoring a change from a previous commit is to copy the current `<filepath>` to `<filepath.bak>`, checkout the hash version of `<filepath>`, and use `vimdiff <filepath> <filepath.bak>` to copy a change over from the previous commit, and then `rm <filepath>` and `mv <filepath.bak> <filepath>`.
